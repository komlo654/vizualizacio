class GeoMap {
    ...
    updateVis() {
        ...
        vis.symbolScale.domain(d3.extent(vis.data, d => d.Capacity));
    ...
    renderVis() {
        ...
        const geoSymbols = vis.chart.selectAll('.geo-symbol')
            ...
            .attr('r', d => vis.symbolScale(d.Capacity / 20))
        ...
        geoSymbols
            .on('mousemove', (event,d) => {
                d3.select('#tooltip')
                    ...
                    .html(`
              <div class="tooltip-title">${d.Stadium}</div>
              <div>${d.Country}&nbsp; | &nbsp;${d.FDCOUK} | &nbsp;${d.Capacity}</div>
            `);
            })
            .on('mouseleave', () => {
        ...
        const geoSymbolLabels = vis.chart.selectAll('.geo-label')
            ...
            .attr('y', d => (vis.projection([d.Longitude,d.Latitude])[1] - vis.symbolScale(d.Capacity) - 8))
            .text(d => d.name);
